...
const ViewGeneralBilling = () => {
  const { id } = useParams();
  const dispatch = useDispatch();
  const [initialData, setInitialData] = useState({});

  const { isGetInitialValuesGeneralBilling, viewGeneralBillingData } =
  ...
  const handleViewGeneralBilling = () => {
    dispatch(
      setIsGetInitialValuesGeneralBilling({
    ...
    dispatch(getGeneralBillingDetail({ billing_id: id, pdf: false }))
      .then(res => {
        ...
        dispatch(setViewGeneralBillingData(updatedData));
      })
      .catch(error => console.error('error', error));
  };

  useEffect(() => {
    if (isGetInitialValuesGeneralBilling?.view === true) {
      setInitialData(viewGeneralBillingData);
    } else {
      handleViewGeneralBilling();
    }
  }, []);
...
export default memo(ViewGeneralBilling);
