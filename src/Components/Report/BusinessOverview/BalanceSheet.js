    ...
    >
      ...
      <div className="bg-white">
        <div className="profit_data d-flex justify-content-between">
          <h5 className="m-0">
            {title === 'Asset' ? 'Gross Profit GP' : 'Net Profit NP'}
          </h5>
          <h5 className="m-0">
            ₹ {title === 'Asset' ? total.toFixed(2) : netProfit}
          </h5>
        </div>
...
const BalanceSheet = () => {
  ...
  const formattedDateRange = useMemo(() => {
    if (balanceSheetStartDate && balanceSheetEndDate) {
      return `${moment(balanceSheetStartDate).format('DD-MM-YYYY')} to ${moment(
        balanceSheetEndDate,
      ).format('DD-MM-YYYY')}`;
    }
    return moment().format('DD-MM-YYYY');
  }, [balanceSheetStartDate, balanceSheetEndDate]);
  ...
  useEffect(() => {
    const startDate =
      balanceSheetStartDate &&
      moment(balanceSheetStartDate).format('YYYY-MM-DD');
    const endDate =
      balanceSheetEndDate && moment(balanceSheetEndDate).format('YYYY-MM-DD');
  ...
  const handleDate = useCallback(
    (e, dateType) => {
      if (e?.value !== null) {
        if (dateType === 'start' && !balanceSheetEndDate) {
          // toast.error('Please select end date');
          return;
        }
        if (dateType === 'end' && !balanceSheetStartDate) {
  ...
  const { income, expense } = balanceSheetReportData;

  // Same grouping as Trading
  ...
  const handleSelectFinancialYear = useCallback(
    ...
    [balanceSheetEndDate, balanceSheetStartDate, fetchRequiredData],
  );
              ...
              <div className="back_page">
                <div className="btn_as_text d-flex align-items-center">
                  ...
                  <div className="page_title d-flex align-items-center">
                    <h2 className="m-0 pe-2">Balance Sheet</h2>
                    ...
                    <div className="date_select text-end">
                      <Calendar
                        ...
                        onClearButtonClick={e => {
                          setStartDateForm('');
                          setEndDateForm('');
                          dispatch(setBalanceSheetStartDate(''));
                          dispatch(setBalanceSheetEndDate(''));
                          handleClearDate();
                        }}
                        readOnlyInput
                      />
